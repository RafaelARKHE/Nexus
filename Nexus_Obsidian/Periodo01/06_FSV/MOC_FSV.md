---
sistema: Nexus
tipo: moc_disciplina
disciplina: Fundamentos do Salvamento Veicular
sigla: FSV
periodo: Periodo01
criado_em: 06/06/2026
atualizado_em: 09/06/2026
---

# MOC — FSV | Fundamentos do Salvamento Veicular
> 1º Período · CFO T16 · CBMPA

---

## Resumo da Disciplina
[[Periodo01/06_FSV/resumo_FSV]]

---

## Documentos Processados

| Arquivo | Conteúdo | Data |
|---|---|---|
| FSV CFO - Aula 01.pdf | Conceitos iniciais, Estatísticas, Segurança, FEAs, Anatomia Veicular | 06/06/2026 |
| FSV CFO - Aula 02.pdf | Fases do Atendimento + Estabilização Veicular | 06/06/2026 |
| FSV CFO - Aula 03.pdf | Considerações com a Vítima, Avaliação (XABCDE), Abordagem, Acesso e Extração | 09/06/2026 |

---

## Pontos-Chave por Tema

### 01 — Conceitos e Cenários
> Origem: `FSV CFO - Aula 01.pdf`

| # | Assunto | Arquivo |
|---|---|---|
| 01 | Conceitos Fundamentais e Tipos de Cenários | [[Periodo01/06_FSV/pontos_chave/01_Conceitos-e-Cenarios/01_conceitos-fundamentais]] |
| 02 | Estatísticas de Sinistros de Trânsito | [[Periodo01/06_FSV/pontos_chave/01_Conceitos-e-Cenarios/02_estatisticas-sinistros]] |

### 02 — Segurança e Ações Iniciais
> Origem: `FSV CFO - Aula 01.pdf`

| # | Assunto | Arquivo |
|---|---|---|
| 01 | Segurança nas Ocorrências — 3 Pilares | [[Periodo01/06_FSV/pontos_chave/02_Seguranca-e-Acoes-Iniciais/01_seguranca-nas-ocorrencias]] |
| 02 | Ações Iniciais e Definição de Zonas | [[Periodo01/06_FSV/pontos_chave/02_Seguranca-e-Acoes-Iniciais/02_acoes-iniciais-e-zonas]] |

### 03 — FEAs
> Origem: `FSV CFO - Aula 01.pdf`

| # | Assunto | Arquivo |
|---|---|---|
| 01 | FEAs — Ferramentas, Equipamentos e Acessórios (visão geral) | [[Periodo01/06_FSV/pontos_chave/03_FEAs/01_feas-visao-geral]] |

### 04 — Anatomia Veicular
> Origem: `FSV CFO - Aula 01.pdf`

| # | Assunto | Arquivo |
|---|---|---|
| 01 | Anatomia Veicular — Estrutura e Componentes Críticos | [[Periodo01/06_FSV/pontos_chave/04_Anatomia-Veicular/01_anatomia-veicular]] |

### 05 — Fases e Estabilização
> Origem: `FSV CFO - Aula 02.pdf`

| # | Assunto | Arquivo |
|---|---|---|
| 01 | Fases do Atendimento de Salvamento Veicular | [[Periodo01/06_FSV/pontos_chave/05_Fases-e-Estabilizacao/01_fases-do-atendimento]] |
| 02 | Estabilização Veicular — Tipos e Formas por Posição | [[Periodo01/06_FSV/pontos_chave/05_Fases-e-Estabilizacao/02_estabilizacao-veicular]] |

### 06 — Abordagem e Extração
> Origem: `FSV CFO - Aula 03.pdf`

| # | Assunto | Arquivo |
|---|---|---|
| 01 | Considerações com a Vítima e Avaliação — Hora de Ouro, XABCDE, Tempos e Planos | [[Periodo01/06_FSV/pontos_chave/06_Abordagem-e-Extracao/01_consideracoes-e-avaliacao-da-vitima]] |
| 02 | Abordagem da Vítima, Acesso ao Veículo e Extração | [[Periodo01/06_FSV/pontos_chave/06_Abordagem-e-Extracao/02_abordagem-acesso-e-extracao]] |

---

## Conexões com outras disciplinas
- [[Periodo01/01_APHR/MOC_APHR]] — APH integrado ao SV: segurança da vítima e primeiros atendimentos na cena
- `Periodo01/??_FCIU/MOC_FCIU` *(pendente)* — incêndio em veículo sinistrado

---

## Avaliações

### VC01
| Artefato | Arquivo |
|---|---|
| Revisão | [[Periodo01/06_FSV/VC01/revisao_VC01]] |
| Áudio | `VC01/audio_VC01.txt` |
| Simulado | `VC01/simulado_VC01.html` |
| Flashcards | `VC01/flashcard_VC01.html` |

---

## Status
- ✅ Aula 01 processada — 6 arquivos de pontos-chave + resumo
- ✅ Aula 02 processada — 2 arquivos de pontos-chave + resumo atualizado
- ✅ Aula 03 processada — 2 arquivos de pontos-chave + resumo atualizado
- ✅ VC01 — todos os artefatos de prova gerados e atualizados com a Aula 03 (simulado ampliado para 20 questões)
